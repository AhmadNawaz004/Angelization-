...
/* Java packages */
public class TotalSupportTree extends Angel {
	...
	protected BT[] addToTtree(BT[] linkRef, int size,
		...
		if (linkRef[currentAttribute] == null)
			linkRef[currentAttribute] = new BT();
	...
	protected int getSupportForItemSetInTtree(short[] itemSet) {
		...
		if (startTtreeRef[itemSet[endInd]] != null) {
			...
			// Otherwise continue down branch
				...
				BT[] tempRef = startTtreeRef[itemSet[endInd]].childRef;
				if (tempRef != null)
	 ...
	 * @param txtNoofFreqSets 
	 */

	public void outputNumFreqSets(JTextField txtNoofFreqSets) {
...
//			System.out.println("Number of frequent sets = "+ countNumFreqSets());
			txtNoofFreqSets.setText(""+countNumFreqSets());
		}
	}
	...
	protected int countNumFreqSets(int size, BT[] linkRef, int num) {
	...
	public void outputTtreeStats() {
		...
		System.out.println(BT.getNumberOfNodes() + " nodes");
	...
	public void outputStorage(JTextArea jtaShowMining) {
		...
		System.out.println();
		jtaShowMining.append("Storage          = " + calculateStorage()+ " (Bytes)"+"\n");
	}
	...
	private int calculateStorage(int localStorage, BT[] linkRef) {
